...
function changeSelectedCourse(plans_data) {
    ...
    if (selectedPlan in plans_data) {
        ...
        const newAmountInWords = `${numberToWords.toWords(
      courseDataObject.price
    )} rupee`;
        ...
        maxDiscountedAllowed =
            courseDataObject.price - courseDataObject.maxDiscountPrice;
        enableFields();
    }
}
...
discount.addEventListener("input", function () {
    ...
    if (discount.value == "") {
        discountedFee.value = `${Number(courseFee.value.split(" ")[0]).toFixed(
      2
    )} ₹`;
    ...
    if (newPaymentAmount) {
        const selectedGstOption = document.querySelector(
            'input[name="gstOption"]:checked'
        );
    ...
    if (balanceAmount) {
        ...
        if (selectedGstOption === "exclude") {
            total.value =
                Number(paymentAmount.value) + Number(cgst.value) + Number(sgst.value);
            if (discountedPriceAmount) {
                balanceAmount = discountedPriceAmount - Number(paymentAmount.value);
            } else {
    ...
    if (isNew) {
        if (startDate.value !== "") {
            ...
            if (!isNaN(inputDate.getTime())) {
                endDate.value = calculateCourseEndDate(
                    startDate.value,
...
paymentAmount.addEventListener("input", function () {
    ...
    // Calculate IGST
    const igstAmount = (paymentAmount.value * 0.0).toFixed(2);
...
function sendData() {
    ...
    //common data field sent only sent tobe for the first time
    const commonData = {
        personalInfo: {
    ...
    if (isNew) {
        SendDataToFlutter.postMessage(JSON.stringify({
            ...commonData,
            ...paymentData
        }));
    } else {
        SendDataToFlutter.postMessage(JSON.stringify(paymentData));
    }
}
